...
ready(async () => {
  ...
  setTimeout(async () => {
    ...
    const anyInactiveLangcode = availableLangcodes.find(
      (langcode) => langcode.code !== activeLangcode,
    );
    if (activeLangcode === "en" && anyInactiveLangcode) {
      console.warn(
        `Set inactive langcode "${anyInactiveLangcode.code}" and back to active langcode "${activeLangcode}"`,
      );
